...
class ItemModelTest(TestCase):
    ...
    def test_saving_and_retrieving_items(self):
        list_user = List.objects.create()  # ✅ 创建 List

        first_item = Item()
        first_item.text = 'The first list item'
        first_item.list = list_user  # ✅ 设置外键
        first_item.save()

        second_item = Item()
        second_item.text = 'Item the second'
        second_item.list = list_user  # ✅ 设置外键
        second_item.save()
...
class NewItemTest(TestCase):
    ...
    def test_redirects_to_list_view(self):
        other_list = List.objects.create()
        corrrect_list = List.objects.create()

        response = self.client.post(
            f'/lists/{corrrect_list.id}/add_item',
            data={'item_text': 'A new item for an existing list'}
        )

        self.assertRedirects(response, f'/lists/{corrrect_list.id}/')
